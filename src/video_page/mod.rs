...
impl RonajoVideoPage {
    ...
    pub fn update_rate(&self) {
        if let Some(duration) = self.playbin().query_duration::<gst::ClockTime>() {
            if let Some(position) = self.playbin().query_position::<gst::ClockTime>() {
                let seek_event = gst::event::Seek::new(self.rate(), gst::SeekFlags::FLUSH, gst::SeekType::Set, position, gst::SeekType::Set, duration);
                let audio_sink = self.playbin().property::<gst::Element>("audio-sink");
                audio_sink.send_event(seek_event);
            } else {
                let seek_event = gst::event::Seek::new(self.rate(), gst::SeekFlags::FLUSH, gst::SeekType::Set, self.prev_pos(), gst::SeekType::Set, duration);
                let audio_sink = self.playbin().property::<gst::Element>("audio-sink");
                audio_sink.send_event(seek_event);
            }
        }
    ...
    pub fn setup_stream(&self) {
            ...
            let playbin = gst::ElementFactory::make("playbin")
                .property("uri", "http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4")
            ...
            self.imp().prev_pos.replace(Some(gst::ClockTime::from_seconds(0)));
            self.imp().last_hidden.replace(Some(Instant::now()));
            self.imp().last_volume_revealed.replace(Some(Instant::now()));
            ...
            let timeout_id =
                glib::timeout_add_local(std::time::Duration::from_millis(500), move || {
                    ...
                    if let Some(duration) = playbin.query_duration::<gst::ClockTime>() {
                        video_slider.adjustment().set_upper(duration.seconds() as f64);
                    };
                    ...
                    if let Some(position) = position {
                        position_label.borrow().clone().set_label(&format!("{}", seconds_to_timestamp(position.seconds() as f64)));
                    }

                    if let Some(duration) = duration {
                        duration_label.borrow().clone().set_label(&format!("{}", seconds_to_timestamp(duration.seconds() as f64)));
                    }
                    ...
                    if let Some(position) = position {
                        video_slider.adjustment().set_value(position.seconds() as f64);
                    }
    ...
    pub fn setup_callbacks(&self) {
        ...
        click_event.connect_pressed(glib::clone!(@weak self as page => move |_,_,_,_| {
        ...
        mouse_motion_event.connect_motion(glib::clone!(@weak self as page => move |_, x, y| {
            if page.prev_xy() == (x, y) {
               return;
            }
            page.imp().last_hidden.replace(Some(Instant::now()));
        ...
        let mouse_scroll_event = gtk::EventControllerScroll::new(gtk::EventControllerScrollFlags::VERTICAL);
        ...
        imp.video_slider.connect_value_changed(glib::clone!(@weak playbin => move |scale| {
            if let Some(position) = playbin.query_position::<gst::ClockTime>() {
                ...
                if difference > 1 {
                    playbin.seek_simple(gst::SeekFlags::FLUSH, gst::ClockTime::from_seconds(value)).expect("failed to seek");
                }
            } else {
                let value = scale.adjustment().value() as u64;
                playbin.seek_simple(gst::SeekFlags::FLUSH, gst::ClockTime::from_seconds(value)).expect("failed to seek");
        ...
        imp.rate_label.connect_label_notify(glib::clone!(@weak self as page => move |_| {
            page.imp().last_rate_revealed.replace(Some(Instant::now()));
        }));

        imp.audio_slider.adjustment().connect_value_changed(glib::clone!(@weak self as page => move |_| {
            page.imp().last_volume_revealed.replace(Some(Instant::now()));
        }));
        let mute_button = self.imp().mute_button.get();
        // let menu_mute_button = self.imp().menu_mute_button.get();

        imp.audio_slider.adjustment().connect_value_changed(glib::clone!(@weak mute_button, @weak self as page => move |_| {
            let volume = page.volume();
    ...
    pub fn setup_binds(&self) {
        ...
        for widget in pages {
            let window = widget.downcast_ref::<gtk::Window>()
                ...
                if window.title() == Some("ronajo".to_string().into()) {
                    window.bind_property("fullscreened", &imp.fullscreen_button.get(), "active")
                        .sync_create()
                        .build();
                }
        }

        imp.play_button.bind_property("active", &imp.play_button.get(), "icon-name")
        ...
        imp.play_button.bind_property("active", &imp.play_button.get(), "tooltip-text")
            .transform_to(|_, active| {
        ...
        imp.fullscreen_button.bind_property("active", &imp.fullscreen_button.get(), "icon-name")
            .transform_to(|_, active| {
        ...
        imp.menu_mute_button.bind_property("active", self, "mute")
            .bidirectional()
            .sync_create()
            .build();
        ...
        imp.lock_ui_button.bind_property("active", self, "autohide")
            .bidirectional()
            .invert_boolean()
            .sync_create()
            .build();

        imp.loop_button.bind_property("active", self, "loop-video")
            .bidirectional()
            .sync_create()
            .build();

        imp.lock_ui_button.bind_property("active", &imp.lock_ui_button.get(), "icon-name")
            .transform_to(|_, active| {
